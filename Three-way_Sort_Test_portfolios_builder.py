...
three_way_sorts=["size_"+("_".join(map(str, comb))) for comb in combinations(all_measures, 2)]
  
# Print the obtained permutations 
print('three-way sorts built on:\n- {}'.format('\n- '.join(three_way_sorts)))
...
test_assets_path="G:\\My Drive\\PhD\\Research\\Indipendent project\\estimated_data\\test_assets\\three_way_sort\\"
...
# Reading the original Fama and French Three-way sort portfolios

# In[9]:


FamaFrench_test_assets_repository={
'size_bm_op':'https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/32_Portfolios_ME_BEME_OP_2x4x4_CSV.zip',
'size_bm_inv':'https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/32_Portfolios_ME_BEME_INV_2x4x4_CSV.zip',
'size_op_inv':'https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/ftp/32_Portfolios_ME_OP_INV_2x4x4_CSV.zip'
                                    }
...
if "assets_downloaded" not in locals():
    ...
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('BIG','ME2')
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('LoBM','BM1')
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('HiBM','BM4')
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('LoINV','INV1')
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('HiINV','INV4')
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('LoOP','OP1')
    ff_test_assets['ptf_code']=ff_test_assets['ptf_code'].str.replace('HiOP','OP4')
...
for f in tqdm(nf_measures_files, desc='measures_files_loop'): 
    ...
    ccm1_jun = pd.merge(ccm_jun, size_ptf, how='left', left_on=['jdate','permno'], right_index=True)
    
    # the following breakpoints are conditional on stock being small or big (sizeport is in the groupby)
    # assign book-to-market portfolios 
    beme_ptf=(ccm1_jun.set_index(['permno']).groupby(['jdate','sizeport'])
                                          .apply(lambda x:
                                                          pd.cut(x['beme'],[-float("inf")]+
                                                                           ...
                                                                           +[float("inf")],
                                                                 labels=['bm1','bm2','bm3','bm4'],
                                                                 right=True, include_lowest=True
                                                                )
                                                ).to_frame()).rename(columns={'beme':'bmport'})

    # assign operating profitability portfolios
    op_ptf=(ccm1_jun.set_index(['permno']).groupby(['jdate','sizeport'])
                                          .apply(lambda x:
    ...
    inv_ptf=(ccm1_jun.set_index(['permno']).groupby(['jdate','sizeport'])
                                          .apply(lambda x:
    ...
    #join back op, beme and inv breakdown
    portfolios = pd.merge(pd.merge(beme_ptf, inv_ptf, how='outer', right_index=True, left_index=True), op_ptf, how='outer', right_index=True, left_index=True).reset_index().set_index(['jdate','permno']).drop(columns='sizeport')

    # allign breakpoints to variables
    ccm1_jun = pd.merge(ccm1_jun, portfolios, how='left', left_on=['jdate','permno'], right_index=True)
    ccm1_jun['ffyear']=ccm1_jun['jdate'].dt.year
    ccm1_jun    
    
#     beme_ptf.groupby(['sizeport','bmport'])['bmport'].value_counts()
#     beme_ptf.groupby(['jdate','sizeport','bmport']).size().to_frame().head(100)
#     ccm1_jun.groupby(['jdate','sizeport','bmport','invport','opport'])['nf'].count()

    print('##################################################################################################################')
    print('### START LOOP ON MEASURES OF FILE: {}'.format(f.split('\\')[-1]))
    print('##################################################################################################################')

#     nf_measures=nf_measures.iloc[:,23].to_frame() #DEBUG: UNCOMMENT
...
#         nf_measure_not_good=0
        ...
        dates_with_at_least_2_nf=ccm1_jun[(ccm1_jun['exchcd']==1)&((ccm1_jun['shrcd']==10) | (ccm1_jun['shrcd']==11))&(~ccm1_jun['nf'].isna())].groupby(['jdate','sizeport'])['nf'].nunique().to_frame()
        dates_with_at_least_2_nf=dates_with_at_least_2_nf[dates_with_at_least_2_nf['nf']>=2].reset_index().drop(columns='nf')
        ccm1_jun_with_at_least_2_valid_nf_for_each_per_period=pd.merge(ccm1_jun, dates_with_at_least_2_nf, how='inner', on=['jdate','sizeport']).reset_index().dropna(subset=['nf'])
        nf_ptf=(ccm1_jun_with_at_least_2_valid_nf_for_each_per_period.dropna(subset=['nf']).set_index(['permno']).groupby(['jdate','sizeport'])
                                              .apply(lambda x: pd.cut(x['nf'], ([-float("inf")]+
                                                                                [l-epsilon*(3-ind-1) for ind, l in 
                                                                                enumerate(
                                                                                x[(x['exchcd']==1)&
                                                                                ((x['shrcd']==10) | (x['shrcd']==11))&                                           
                                                                                (~x['nf'].isna())]['nf']
                                                                                ...
                                                                                +[float("inf")]),
                                                                     labels=['nf1','nf2','nf3','nf4'],
                                                                     right=True, include_lowest=True
                                                                    )
                                                    ).to_frame()).reset_index().rename(columns={'nf':'nfport'}).drop(columns='sizeport')
    ...
    #         print(pd.pivot(nf_bp, index='jdate', columns='nf_bp', values='nf').add_prefix('nf_'))
        ...
        ################################
        ## Three-way sorts portfolios ##
        ################################
        print('################################\n## Three-way sorts portfolios ##\n################################')
    
        #loop on the triplets of measures for the three-way sort
#         for sort_on in [three_way_sorts[0]]: #DEBUG: selecting
        for sort_on in three_way_sorts:
...
#                     continue
    ...
    ptfs_nfirms2export.to_csv(test_assets_path+f.split('\\')[-1].split('.')[0].replace('measures_','three_way_sorts_test_portfolios_nfirms_')+'.csv', index = True, float_format='%.6f')
    ptfs2export.to_csv(test_assets_path+f.split('\\')[-1].split('.')[0].replace('measures_','three_way_sorts_test_portfolios_')+'.csv', index = True, float_format='%.6f')
    measures_legend_f.to_csv(test_assets_path+f.split('\\')[-1].split('.')[0].replace('measures_','measures_legend_three_way_sorts_test_portfolios_')+'.csv', index = False, float_format='%.6f')
...
get_ipython().system('jupyter nbconvert --to script Three-way_Sort_Test_portfolios_builder.ipynb')
